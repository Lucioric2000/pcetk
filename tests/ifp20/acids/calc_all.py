...
class TrainingSet(object):
    ...
    def __init__(self, rootDir, acidDirs, method, useLowLevelEel=False):
        ...
        for acidDir in acidDirs:
            prot = ProtonationForm(
                ...
                fileHess="%s/%s/prot/%s/prot_%s.out"
                % (rootDir, acidDir, method, method),
                fileCbs="%s/%s/prot/%s/cbs/prot_%s_cbs.out"
                % (rootDir, acidDir, method, method),
                fileCosmo="%s/%s/prot/%s/cosmo/prot_%s_cosmo.out"
                % (rootDir, acidDir, method, method),
            )
            prot.ParseFiles()
            ...
            deprot = ProtonationForm(
                label="%s" % acidDir.replace("_", "-"),
                fileHess="%s/%s/deprot/%s/deprot_%s.out"
                % (rootDir, acidDir, method, method),
                fileCbs="%s/%s/deprot/%s/cbs/deprot_%s_cbs.out"
                % (rootDir, acidDir, method, method),
                fileCosmo="%s/%s/deprot/%s/cosmo/deprot_%s_cosmo.out"
                % (rootDir, acidDir, method, method),
            )
            deprot.ParseFiles()
    ...
    def CorrectAcids(self):
        ...
        nacids = len(self.acids)
        self.sigma = (1.0 / nacids * sigma) ** 0.5
    ...
    def WriteTable(self, filename="fit.dat", sortByExperiment=False):
        ...
        for i in sequence:
            acid = self.acids[i]
            fo.write(
                '%14.1f%14.1f%14.1f%14.1f%14.1f     "%s"\n'
                % (
                    acid.pKa_exp,
                    acid.pKa,
                    acid.DeltaG_corr,
