...
class ProtonationForm(object):
    ...
    def ParseFiles(self):
        ...
        if not self.isParsed:
            ...
            self.Eel = (
                self._Parser(self.fileHess, ((pattern, -1),))[pattern]
                * UNITS_ENERGY_HARTREES_TO_KILOCALORIES_PER_MOLE
            )
            pattern = "G-E(el)"
            self.Gcorr = (
                self._Parser(self.fileHess, ((pattern, 2),))[pattern]
                * UNITS_ENERGY_HARTREES_TO_KILOCALORIES_PER_MOLE
            )
            pattern = "Total Energy after outlying charge correction"
            self.Eel_cosmo = (
                self._Parser(self.fileCosmo, ((pattern, -2),))[pattern]
                * UNITS_ENERGY_HARTREES_TO_KILOCALORIES_PER_MOLE
            )
            # Level: MP2/CBS
            pattern = "FINAL SINGLE POINT ENERGY"
            self.Eel_cbs = (
                self._Parser(self.fileCbs, ((pattern, -1),))[pattern]
                * UNITS_ENERGY_HARTREES_TO_KILOCALORIES_PER_MOLE
            )
            self.isParsed = True
    ...
    def CalculateTerms(self, useLowLevelEel=False):
        ...
        if self.isParsed:
            ...
            if useLowLevelEel:
                self.Gaq = self.Eel + self.Esol + self.Gcorr
            else:
                self.Gaq = self.Eel_cbs + self.Esol + self.Gcorr
    ...
    def CalculateDeltas(self, other):
        ...
        if self.isCalcTerms:
            ...
            self.pKa = self.DeltaGaq / (
                CONSTANT_MOLAR_GAS_KCAL_MOL * self.temperature * CONSTANT_LN10
            )
