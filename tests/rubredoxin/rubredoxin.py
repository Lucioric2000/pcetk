...
from ContinuumElectrostatics import (
    MEADModel,
    StateVector,
    TitrationCurves,
    MCModelDefault,
...
parameters = [
    "toppar/par_all27_prot_na_fixed.inp",
    "toppar/par_iron_sulfur.inp",
]

mol = CHARMMPSFFile_ToSystem(
    "rubredoxin_xplor.psf",
    isXPLOR=True,
...
electrostaticModel = MEADModel(
    system=mol, pathMEAD="/home/mikolaj/local/bin/", pathScratch="mead", nthreads=1
)
...
for mcModel, folded, direc, message, sedFile, substateFile in (
    (
        ...
        "curves_analytic",
        "analytically",
        "prob_ph7_analytic.sed",
        "substate_ph7_analytic.tex",
    ),
    (
        mcModelGMCT,
        True,
        ...
        "using GMCT",
        "prob_ph7_gmct.sed",
        "substate_ph7_gmct.tex",
    ),
    (
        mcModelDefault,
        True,
        "curves_custom",
        "using custom MC sampling",
        "prob_ph7_custom.sed",
        "substate_ph7_custom.tex",
    ),
):
    electrostaticModel.DefineMCModel(mcModel)

    logFile.Text("\n***Calculating titration curves %s***\n" % message)
    curves = TitrationCurves(electrostaticModel, curveSampling=0.5)
    curves.CalculateCurves()
    curves.WriteCurves(directory=direc)

    logFile.Text("\n***Calculating protonation states at pH=7 %s***\n" % message)
    electrostaticModel.CalculateProbabilities(pH=7.0)
...
vector.Reset()
for i in range(10):
    Gmicro = electrostaticModel.CalculateMicrostateEnergy(vector, pH=7.0)
    vector.Increment()
    table.Entry("%d" % (i + 1))
