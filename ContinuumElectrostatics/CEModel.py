...
_DEFAULT_FOCUSING_STEPS = ((121, 2.0), (101, 1.0), (101, 0.5), (101, 0.25))
_DEFAULT_EXCLUDE_SEGMENTS = (
    "WATA",
    "WATB",
    "WATC",
    "WATD",
...
_DEFAULT_TEMPERATURE = 300.0  # 300 K
_DEFAULT_IONIC_STRENGTH = 0.1  # 100 mM = 0.1 M
_DEFAULT_EPSILON_WATER = 80.0
_DEFAULT_EPSILON_PROTEIN = 4.0
...
class CEModel(object):
    ...
    def Initialize(
        self,
        excludeSegments=_DEFAULT_EXCLUDE_SEGMENTS,
        ...
        if not self.isInitialized:
            ...
            totalSites, totalInstances = self._SplitModel(
                excludeSegments=excludeSegments,
                excludeResidues=excludeResidues,
    ...
    # -------------------------------------------------------------------------------
    def SummaryProbabilities(
        self, reportOnlyUnusual=False, maxProbThreshold=0.75, log=logFile
    ):
    ...
    # -------------------------------------------------------------------------------
    def SedScript_FromProbabilities(
        self, filename="his_repl.sed", overwrite=False, putPath=False, log=logFile
    ):
            ...
            if site.resName not in ("HIS", "HSP"):
                ...
                if site.resName in unusualProtonations:
                    ...
                    if mostProbLabel in unusualLabels:
                        if site.resName == "ASP":
                            lines.append(
                                "# patch ASPP %4s %4d setup  ! %.4f\n"
                                % (site.segName, site.resSerial, mostProbValue)
                            )
                        elif site.resName == "GLU":
                            lines.append(
                                "# patch GLUP %4s %4d setup  ! %.4f\n"
                                % (site.segName, site.resSerial, mostProbValue)
                            )
                        else:
                            warnings.append(
                                "# Warning: %4s %3s %4d is %s with probability of %.4f\n"
                                % (
                                    site.segName,
                                    site.resName,
                                    site.resSerial,
                                    translateLabels[mostProbLabel],
                                    mostProbValue,
                                )
                            )
                else:
                    warnings.append(
                        '# Unknown residue: %4s %3s %4d (most probable instance is "%s" with probability of %.4f)\n'
                        % (
                            site.segName,
                            site.resName,
                            site.resSerial,
                    ...
                    for ainstance in asite.instances:
                        ...
                        for bsite in self.sites:
                            for binstance in bsite.instances:
                                Wij = self.energyModel.GetInteractionSymmetric(
                                    ainstance._instIndexGlobal,
                                    binstance._instIndexGlobal,
                                )
        ...
        if self.isCalculated:
            ...
            for asite in self.sites:
                for ainstance in asite.instances:
                    ...
                    for bsite in self.sites:
                        for binstance in bsite.instances:
                            interSymmetric = self.energyModel.GetInteractionSymmetric(
                                ainstance._instIndexGlobal, binstance._instIndexGlobal
                            )
                            interaction = self.energyModel.GetInteraction(
                                ainstance._instIndexGlobal, binstance._instIndexGlobal
                            )
                            deviation = self.energyModel.GetDeviation(
                                ainstance._instIndexGlobal, binstance._instIndexGlobal
                            )
                            lines.append(
                                form
                                % (
                                    asite.siteIndex + 1,
                                    ainstance.instIndex + 1,
                                    asite.label,
                                    ainstance.label,
                                    bsite.siteIndex + 1,
                                    binstance.instIndex + 1,
                                    bsite.label,
                                    binstance.label,
                                    interSymmetric,
                                    interaction,
    ...
    def CalculateProbabilities(
        ...
        if hasattr(self, "sampler") and unfolded:
            raise ContinuumElectrostaticsError(
                "Monte Carlo sampling of unfolded proteins unsupported."
            )
        elif hasattr(self, "sampler") and not unfolded:
            self.sampler.CalculateOwnerProbabilities(
                pH=pH,
            ...
            if trajectoryFilename != "":
                raise ContinuumElectrostaticsError(
                    "Writing trajectories of unfolded proteins unsupported."
                )
    ...
    def _GetIndices(self, residue, atomLabels, check=True):
        ...
        for label in atomLabels:
            ...
            if check and missingLabels:
                ...
                raise ContinuumElectrostaticsError(
                    "Cannot include residue %s %s %d because of missing atoms: %s"
                    % (segmentName, residueName, residueSerial, " ".join(missingLabels))
                )
        return atomIndices
    ...
    # -------------------------------------------------------------------------------
    def _SetupSites(
        self, residue, prevResidue=None, nextResidue=None, terminal=None, log=logFile
    ):
        segmentName, residueName, residueSerial = self._GetResidueInfo(residue)
        setupSites = []
        if terminal:
        ...
        if residueName in self.library:
            ...
            if not terminal:
                prevIndices = []
                if prevResidue:
                    foo, prevResidueName, prevResidueSerial = self._GetResidueInfo(
                        prevResidue
                    )
                    if prevResidueName in PROTEIN_RESIDUES:
                        prevIndices = self._GetIndices(
                            prevResidue, PREV_RESIDUE, check=False
                        )
                    modelIndices = prevIndices + modelIndices

                nextIndices = []
                if nextResidue:
                    foo, nextResidueName, nextResidueSerial = self._GetResidueInfo(
                        nextResidue
                    )
                    if nextResidueName in PROTEIN_RESIDUES:
                        ...
                        nextIndices = self._GetIndices(
                            nextResidue, nextLabels, check=False
                        )
        ...
        for segment in segments:
            ...
            if segmentName not in excludeSegments:
                ...
                for residueIndex, residue in enumerate(residues):
                    foo, residueName, residueSerial = self._GetResidueInfo(residue)
                    includeResidue = self._CheckResidue(
                        excludeResidues,
                        segmentName,
                        residueName,
                        residueSerial,
                    ...
                    setupSites = self._SetupSites(
                        residue,
                        prevResidue,
                        nextResidue,
                        terminal=(
                            terminal
                            if (includeTermini and (residueName in PROTEIN_RESIDUES))
                            else None
                        ),
                        log=log,
                    )
    ...
    # -------------------------------------------------------------------------------
    def _CheckResidue(
        self, excludeResidues, segmentName, residueName, residueSerial, log=logFile
    ):
        ...
        if excludeResidues:
            for exclSegmentName, exclResidueName, exclResidueSerial in excludeResidues:
                if (exclSegmentName) and (exclResidueName) and (exclResidueSerial):
                    if (
                        exclSegmentName == segmentName
                        and exclResidueName == residueName
                        and exclResidueSerial == residueSerial
                    ):
                        includeResidue = False
                        break

                elif (
                    (exclSegmentName) and (exclResidueName) and (not exclResidueSerial)
                ):
                    if (
                        exclSegmentName == segmentName
                        and exclResidueName == residueName
                    ...
                    (exclSegmentName) and (not exclResidueName) and (exclResidueSerial)
                ):
                    if (
                        exclSegmentName == segmentName
                        and exclResidueSerial == residueSerial
                    ...
                    (exclSegmentName)
                    and (not exclResidueName)
                    and (not exclResidueSerial)
                ):
                    ...
                    (not exclSegmentName) and (exclResidueName) and (exclResidueSerial)
                ):
                    if (
                        exclResidueName == residueName
                        and exclResidueSerial == residueSerial
                    ):
                        includeResidue = False
                        break
