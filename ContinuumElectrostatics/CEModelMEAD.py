...
class CEModelMEAD(CEModel):
    ...
    def __init__(self, system, customFiles=None, log=logFile, **keywordArguments):
        """Constructor."""
        super(CEModelMEAD, self).__init__(
            system, customFiles=customFiles, log=log, **keywordArguments
        )
    ...
    def _CreateSite(self, **keywordArguments):
        ...
        updatedIndexGlobal = newSite._CreateInstances(
            libSite.instances, instIndexGlobal
        )
    ...
    # -------------------------------------------------------------------------------
    def CalculateElectrostaticEnergies(
        self,
        calculateETA=False,
        asymmetricTolerance=0.05,
        asymmetricSummary=False,
        log=logFile,
    ):
        if self.isFilesWritten:
            ...
            if self.nthreads < 2:
                ...
                for batch in batches:
                    ...
                    for thread in batch:
                        ...
                        instance._TableEntry(
                            tab, secondsToCompletion=secondsToCompletion
                        )
            if tab:
                tab.Stop()
            ...
            self._CheckIfSymmetric(
                tolerance=asymmetricTolerance, printSummary=asymmetricSummary, log=log
            )
    ...
    def _CheckIfSymmetric(self, tolerance=0.05, printSummary=False, log=logFile):
        ...
        isSymmetric, maxDeviation = self.energyModel.CheckIfSymmetric(
            tolerance=tolerance
        )
                            ...
                            for columnSite in self.sites:
                                for columnInstance in columnSite.instances:
                                    deviation = self.energyModel.GetDeviation(
                                        rowInstance._instIndexGlobal,
                                        columnInstance._instIndexGlobal,
                                    )
                                    if abs(deviation) > tolerance:
                                        report.append(
                                            [rowInstance, columnInstance, deviation]
                                        )

                    for ainstance, binstance, deviation in report:
                        asite = ainstance.parent
                        for gap, content in zip(
                            gaps,
                            (
                                asite.segName,
                                asite.resName,
        ...
        if self.isInitialized:
                ...
                if radii.has_key(atomType):
                    ...
                    if radii.has_key(generalAtomType):
                        ...
                        raise ContinuumElectrostaticsError(
                            "Cannot find atomic radius for atom type %s" % atomType
                        )
            ...
            if not os.path.exists(self.pathScratch):
                try:
                    ...
                    raise ContinuumElectrostaticsError(
                        "Cannot create scratch directory %s" % self.pathScratch
                    )
            ...
            for siteIndex, site in enumerate(self.sites):
                for instanceIndex, instance in enumerate(site.instances):
                    for atomIndex, charge in zip(
                        site.siteAtomIndices, instance.charges
                    ):
