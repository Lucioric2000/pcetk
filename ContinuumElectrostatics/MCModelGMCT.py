...
class MCModelGMCT(object):
    ...
    def CalculateOwnerProbabilities(
        self, pH=7.0, dryRun=False, logFrequency=None, log=logFile
    ):
        ...
        project = "job"
        potential = (
            -CONSTANT_MOLAR_GAS_KCAL_MOL * owner.temperature * CONSTANT_LN10 * pH
        )
        fileContent = _DefaultSetupGMCT % (
            self.tripleFlip,
            self.productionScans,
            owner.temperature,
            self.doubleFlip,
            self.equilibrationScans,
            potential,
            potential,
        )
        ...
        if not os.path.exists(dirConf):
            os.makedirs(dirConf)
        if not os.path.exists(dirCalc):
            os.makedirs(dirCalc)
        if not os.path.exists(fileGint):
            owner.WriteGintr(fileGint, precision=8)
        if not os.path.exists(fileInter):
            owner.WriteW(fileInter, precision=8)
        if not os.path.exists(fileConf):
            WriteInputFile(fileConf, ["conf  0.0  0.0  0.0\n"])
        if not os.path.exists(fileSetup):
            WriteInputFile(fileSetup, fileContent)
        if not os.path.exists(linkname):
            os.symlink("../conf", linkname)
        ...
        if not dryRun:
            ...
            if os.path.exists(os.path.join(dirCalc, output)):
                ...
                try:
                    ...
                    raise ContinuumElectrostaticsError(
                        "Failed running command: %s" % " ".join(command)
                    )
