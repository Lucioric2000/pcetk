        ...
        if model.isFilesWritten:
            if os.path.exists(self.modelLog):
                ...
                command = [
                    ...
                    "%f" % model.temperature,
                    "-ionicstr",
                    "%f" % model.ionicStrength,
                    "-epsin",
                    "%f" % model.epsilonProtein,
                    "-epsext",
                    "%f" % model.epsilonWater,
                    instancePqr,
                    modelBackgroundPqr,
                ]
                try:
            ...
            if not all(checks):
                raise ContinuumElectrostaticsError(
                    "Output file %s empty or corrupted. Empty the scratch directory and start anew."
                    % self.modelLog
                )
        ...
        if model.isFilesWritten:
            ...
            for site in interactions:
                for instance in site:
                    energy = instance
                    model.energyModel.SetInteraction(
                        self._instIndexGlobal, indexGlobal, energy
                    )
