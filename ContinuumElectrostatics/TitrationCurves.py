...
class CurveThread(threading.Thread):
    ...
    def run(self):
        ...
        curves = self.curves
        model = curves.owner
        self.sites = model.CalculateProbabilities(
            pH=self.pH, log=None, isCalculateCurves=True, unfolded=curves.unfolded
        )
...
class TitrationCurves(object):
    ...
    def __init__(self, meadModel, log=logFile, *arguments, **keywordArguments):
        ...
        if not meadModel.isCalculated:
            raise ContinuumElectrostaticsError(
                "First calculate electrostatic energies."
            )
    ...
    def _ProbabilitiesSave(self):
        ...
        probabilities = [0.0] * meadModel.ninstances

        for site in meadModel.sites:
            for instance in site.instances:
                probabilities[instance._instIndexGlobal] = energyModel.GetProbability(
                    instance._instIndexGlobal
                )
    ...
    def _ProbabilitiesRestore(self, probabilities):
        ...
        for site in meadModel.sites:
            for instance in site.instances:
                energyModel.SetProbability(
                    instance._instIndexGlobal, probabilities[instance._instIndexGlobal]
                )
        ...
        if not self.isCalculated:
            ...
            if nthreads < 2 or forceSerial:
                for step in range(self.nsteps):
                    sites = owner.CalculateProbabilities(
                        pH=(self.curveStart + step * self.curveSampling),
                        log=None,
                        isCalculateCurves=True,
            ...
            # Parallel run?
                ...
                for step in range(self.nsteps):
                    batch.append(
                        CurveThread(self, self.curveStart + step * self.curveSampling)
                    )
                    if len(batch) > limit:
                ...
                if isinstance(sampler, MCModelGMCT):
                    for step in range(self.nsteps):
                        sampler.CalculateOwnerProbabilities(
                            pH=(self.curveStart + step * self.curveSampling),
                            dryRun=True,
                            log=None,
                        )
                ...
                for batch in batches:
                    ...
                    for thread in batch:
                        ...
                        if tab:
                            tab.Entry("%10d" % step)
                            tab.Entry(
                                "%10.2f" % (self.curveStart + step * self.curveSampling)
                            )
                        step = step + 1
