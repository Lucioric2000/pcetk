...
class Substate(object):
    ...
    def __init__(self, meadModel, selectedSites, pH=7.0, log=logFile):
        ...
        for (
            ...
            for siteIndex, site in enumerate(meadModel.sites):
                if (
                    site.segName == selectedSegment
                    and site.resSerial == selectedResidueSerial
                ):
            ...
            if not foundSite:
                raise ContinuumElectrostaticsError(
                    ...
                    % (selectedSegment, selectedResidueName, selectedResidueSerial)
                )
        ...
        if meadModel.isProbability:
            ...
            for site in meadModel.sites:
                for instance in site.instances:
                    ...
                    ] = energyModel.GetProbability(instance._instIndexGlobal)
            ...
            for site in meadModel.sites:
                for instance in site.instances:
                    energyModel.SetProbability(
                        instance._instIndexGlobal,
                        probabilities[instance._instIndexGlobal],
                    )
        ...
        if self.isCalculated:
            ...
            if LogFileActive(log):
                ...
                for siteIndex in indicesOfSites:
                    ...
                    tab.Heading(
                        "%s %s %d" % (site.segName, site.resName, site.resSerial)
                    )
    ...
    def Summary_ToLatex(
        ...
        if self.isCalculated:
            ...
            lines = [
                "\\begin{tabular}{@{\\extracolsep{2mm}}cc%sc}" % ("l" * nsites),
            ]
